<?php
// Set konfigurasi laporan kesalahan untuk menonaktifkan pesan notice
error_reporting(~E_NOTICE);
session_start(); // Mulai sesi PHP

// Include file konfigurasi dan kelas database
include 'config.php';
include 'includes/db.php';

// Inisialisasi objek database dengan informasi dari file konfigurasi
$db = new DB($config['server'], $config['username'], $config['password'], $config['database_name']);
// Include file fungsi umum
include 'includes/general.php';
// Include file untuk pagination (paging)
include 'includes/paging.php';

// Ambil nilai dari parameter 'm' (modul) dan 'act' (aksi)
$mod = get('m');
$act = get('act');

// Inisialisasi nilai untuk konsisten pada metode penilaian (nilai Random Index)
$nRI = array(
    1 => 0,
    2 => 0,
    3 => 0.58,
...
// Mendapatkan daftar alternatif dari database berdasarkan tahun/periode
...
// Inisialisasi array asosiatif $ALTERNATIF untuk menyimpan data alternatif
foreach ($rows as $row) {
    $ALTERNATIF[$row->kode_alternatif] = $row->nama_alternatif;
}

// Mendapatkan daftar kriteria dari database berdasarkan tahun/periode
...
// Inisialisasi array asosiatif $KRITERIA untuk menyimpan data kriteria
foreach ($rows as $row) {
...
function TOPSIS_hasil_analisa($echo = true)
{
    ...
    // Urutkan array berdasarkan kode alternatif
    ...
    foreach ($data[key($data)] as $key => $value) {
        $r .= "<th class='text-center'>" . $KRITERIA[$key]['nama_kriteria'] . "</th>";
        $no++;
    }
